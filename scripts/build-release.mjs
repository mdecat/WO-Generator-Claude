/**
 * build-release.mjs — Build the React app and package it as a valid
 * Power Platform solution ZIP ready to import into any D365 FS org.
 *
 * Usage:  node scripts/build-release.mjs
 * Output: gbbwogeneratorclaude_v<version>.zip
 *
 * Requirements: Node.js 18+, Python 3 (for ZIP creation)
 */
...
const PUBLISHER_UNIQUE_NAME = 'gbb';
const PUBLISHER_PREFIX      = 'gbb';
const WR_FOLDER             = 'wogenerator';   // → gbb_wogenerator/...
...
const WR_TYPES = {
  ...
  '.png': 5, '.jpg': 6, '.gif': 7, '.ico': 10, '.resx': 12,
  // SVG (type 11) excluded — causes NullRef in some orgs
};
...
function makeWrFileName(logicalName, guid) {
  return logicalName.replace(/\//g, '').replace(/\./g, '') + guid.toUpperCase();
}

console.log(`
╔══════════════════════════════════════════════════════════╗
║   GBB WO Generator Claude — Release Builder              ║
╚══════════════════════════════════════════════════════════╝`);
...
execSync('npm run build', {
  ...
  env: { ...process.env, VITE_FORCE_XRM: 'true' },
});
...
// ── Step 3: Generate solution XML ─────────────────────────────────────────────
console.log('\n▶ 3/4  Generating solution package...');

const SOL_TEMP = join(ROOT, '.sol_release');
...
const solutionXml = `<ImportExportXml version="${ORG_VERSION}" SolutionPackageVersion="9.2" languagecode="1033" generatedBy="CrmLive" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" OrganizationVersion="${ORG_VERSION}" OrganizationSchemaType="Full" CRMServerServiceabilityVersion="9.2.26032.00144">
  <SolutionManifest>
    ...
    <Publisher>
      ...
      <LocalizedNames>
        <LocalizedName description="GBB" languagecode="1033" />
      </LocalizedNames>
      <Descriptions>
        <Description description="Microsoft Global Black Belt" languagecode="1033" />
      </Descriptions>
...
const OUTPUT_ZIP = join(ROOT, `${SOLUTION_NAME}_v${SOL_VERSION.replace(/\./g, '_')}.zip`);
try { rmSync(OUTPUT_ZIP, { force: true }); } catch { /* ignore */ }
...
let pyScript = `import zipfile\nwith zipfile.ZipFile(r'${OUTPUT_ZIP.replace(/\\/g, '\\\\')}', 'w', zipfile.ZIP_DEFLATED) as z:\n`;
for (const [entry, fp] of pyEntries) {
  pyScript += `  z.write(r'${fp.replace(/\\/g, '\\\\')}', '${entry}')\n`;
}
const PY_PATH = join(ROOT, '.makzip_release.py');
writeFileSync(PY_PATH, pyScript, 'utf8');
execSync(`python "${PY_PATH}"`, { stdio: 'inherit' });

// Cleanup temp files
rmSync(SOL_TEMP, { recursive: true, force: true });
rmSync(PY_PATH, { force: true });

const zipName = `${SOLUTION_NAME}_v${SOL_VERSION.replace(/\./g, '_')}.zip`;
console.log(`   ✓ ${zipName}`);
...
║   3. Solutions → Import → Upload ZIP                     ║
║   4. Complete the import wizard                          ║
║                                                          ║
║   OPEN THE APP:                                          ║
║   Solutions → GBB WO Generator Claude                   ║
║   → Web Resources → gbb_wogenerator/index.html           ║
║   → Preview (or add to D365 sitemap)                     ║
╚══════════════════════════════════════════════════════════╝
`);
