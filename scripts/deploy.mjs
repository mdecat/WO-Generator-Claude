/**
 * deploy.mjs — Build the React app, generate a Power Platform solution,
 * and import it to decat.crm.dynamics.com via pac CLI.
 *
 * Creates a valid Power Platform solution ZIP directly (matching the format
 * produced by Dataverse solution export) and imports it via pac CLI.
 *
...
// Publisher that exists in decat org
...
/**
 * WebResources filename in ZIP:
 *   logicalName with '/' and '.' removed → base
 *   + GUID uppercase (with hyphens, matching format from real solution exports)
 * e.g. mdecat_wogenerator/index.html → mdecat_wogeneratorindexhtml + GUID
 */
...
// Skip SVG — Dataverse type 11 import fails on this org with NullReferenceException.
// favicon.svg is cosmetic; leaflet-draw spritesheet has a PNG fallback in CSS.
const webResources = distFiles
  ...
  .map(filePath => {
    ...
    const guidLower = randomUUID();                      // lowercase for <WebResourceId>
    const guid = guidLower.toUpperCase();               // uppercase for FileName
...
// ── customizations.xml ────────────────────────────────────────────────────
// Format matches Dataverse solution export (verified against GBBClaude export)
...
// ── [Content_Types].xml ───────────────────────────────────────────────────
// Format matches Dataverse solution export: BOM + namespace + Override per file
...
// ── WebResources/ — actual binary files with GUID-based names ─────────────
// Naming: logicalName with / and . removed + GUID uppercase (matches D365 export format)
...
console.log('\n▶ 4/4  Importing solution to decat.crm.dynamics.com...');
...
// ZipFile::CreateFromDirectory on Windows produces backslash paths which fail import.
const pyEntries = [
...
// Ensure decat auth profile is active
execSync(`"${PAC}" auth select --name decat-profile`, { stdio: 'inherit' });

// Import via pac CLI
console.log(`   ↑ Uploading to decat.crm.dynamics.com...`);
